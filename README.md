...
>> - [x] ___崩坏·星穹铁道___
>> - [x] ___明日方舟·终末地___
>> - [ ] ___极限竞速·地平线5/6（纵横比被强制固定）___
>> - [ ] ___腐烂国度2（游戏会强制重置分辨率）___
>> - [ ] ___狙击手·幽灵战士契约1/2（游戏中部分画面元素拉伸）___
...
<sup>__点击 `?` 按钮后，通过鼠标点击显示器中其他窗口，可进行窗口捕获__</sup></br>
<sup>__若未显示窗口信息/窗口信息未更新，即该窗口处于内置白名单中（通常为系统核心进程产生的窗口）__</sup>

### __功能__
- [x] __支持目标窗口选择（使用 `?` 按钮）__
- [x] __支持自定义窗口大小参数__
   - [x] __参数支持存储至配置文件（使用 `+` 和 `-` 按钮）__
   - [x] __配置文件中可存储多个大小参数，支持顺序切换（使用 `<` 和 `>` 按钮）__
- [x] __支持自动检测屏幕空间，窗口大小不会超过窗口所在屏幕的最大可显示范围__
   - [x] __窗口将在当前所在屏幕置中__
   - [x] __支持多显示器环境，目标窗口在哪个屏幕中的显示部分最大，则认定处于哪个屏幕中（可通过手动调整窗口位置实现显示器自适应）__ 
